...
| ⏹ 停止 | 终止本轮引擎进程——**本轮作废**：不进 Guard、无提交、不沉淀知识，事件留痕 `status=stopped`。误发了、方向不对了，随时踩刹 |
| 输入框 | Enter 发送；**Shift+Enter 换行**（自动长高，三行封顶）；多行需求描述直接写 |

**知识沉淀**：每轮对话跑完，Tram 确定性抄录三本账到 `.tram/knowledge/`——`requirements.md`（你的每条消息都是需求陈述）、`changes.md`（每笔轨内提交：路径 + 知识域归属）、`risks.md`（每次越界拦截：CR 与裁决指引）。纯事实抄录不解读，人可随时编辑。
